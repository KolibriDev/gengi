    ...
    it('should respond with 404 and message', done => {
      agent
        ...
        .expect(res => {
          assert(
            res.body.statusText,
            `expected text, got ${res.body.statusText}`
          )
        })
        .end(err => {
          if (err) {
  ...
  describe('GET /calculate/usd/22', () => {
    ...
    it('should have a valid currency object', done => {
      agent
        ...
        .expect(res => {
          assert(
            res.body.hasOwnProperty('currency'),
            'respond should have a currency property'
          )
        })
        .expect(res => {
          ...
          assert(
            ...
            `expected currency to be an object, got '${responseType}'`
          )
        })
        .expect(res => {
          assert(
            res.body.currency.hasOwnProperty('rate'),
            'expected currency to have property rate'
          )
        })
        .expect(res => {
          const currency = res.body.currency
          assert(
            currency.hasOwnProperty('code') &&
              currency.hasOwnProperty('name') &&
              currency.hasOwnProperty('rate'),
    ...
    it('should have an ISKvalue number, matching path', done => {
      agent
        ...
        .expect(res => {
          assert(
            res.body.hasOwnProperty('ISKvalue'),
            'respond should have a ISKvalue'
          )
        })
        .expect(res => {
          assert(
            typeof res.body.ISKvalue === 'number',
            'ISKvalue should be a number'
          )
        })
        .expect(res => {
    ...
    it('should have an USDvalue number, calculated value', done => {
      agent
        ...
        .expect(res => {
          assert(
            res.body.hasOwnProperty('USDvalue'),
            'respond should have a USDvalue'
          )
        })
        .expect(res => {
          assert(
            typeof res.body.USDvalue === 'number',
            'USDvalue should be a number'
          )
        })
        .expect(res => {
          assert(
            res.body.USDvalue === res.body.currency.rate * 22,
            'USDvalue should be calculated value'
          )
        })
        .end(err => {
          if (err) {
