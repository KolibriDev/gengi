  ...
  describe('GET /search/króna', () => {
    ...
    it('should have a currencies object containing DKK', done => {
      agent
        ...
        .expect(res => {
          assert(
            res.body.hasOwnProperty('currencies'),
            'respond should have a currencies'
          )
        })
        .expect(res => {
          const responseType = Object.prototype.toString.call(
            res.body.currencies
          )
          assert(
            responseType === '[object Object]',
            `expected currencies to be an object, got '${responseType}'`
          )
        })
        .expect(res => {
          assert(
            res.body.currencies.hasOwnProperty('DKK'),
            'currencies should contain DKK'
          )
        })
        .end(err => {
          if (err) {
