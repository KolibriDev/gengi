...
describe('Helpers > values', () => {
  describe('name(name)', () => {
    it('should exist', done => {
      assert(
        ...
        "expected 'values' to have property 'name'"
      )
      done()
    })
    ...
    it('should be a function', done => {
      assert(
        ...
        'expected values.name to be a function'
      )
      done()
    })
    ...
    it('should return correctly formatted name', done => {
      assert(
        name === 'Sterlingspund',
        `expected 'Sterlingspund', got '${name}'`
      )
      done()
    })
    ...
    it('should return sterlingspund', done => {
      assert(
        nocomma === 'Sterlingspund',
        `expected 'Sterlingspund', got '${nocomma}'`
      )
      done()
    })
  })
  ...
  describe('rate(value)', () => {
    it('should exist', done => {
      assert(
        values.hasOwnProperty('rate'),
        "expected 'values' to have property 'rate'"
      )
      done()
    })
    ...
    it('should be a function', done => {
      assert(
        typeof values.rate === 'function',
        'expected values.rate to be a function'
      )
      done()
    })
    ...
    it('should return a float', done => {
      assert(
        !isNaN(rate) && rate.toString().indexOf('.') !== -1,
        'expected a float'
      )
      done()
    })
