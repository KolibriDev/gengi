...
// let swiftclick = new SwiftClick(document.querySelector('numpad'));
...
let _gengi = {
  ...
  init: () => {
    ...
    Vue.filter('icelandicNumber', {
      read: function(value) {
        return format.numberIcelandic(value);
      },
    ...
    _gengi.vm = new Vue({
      ...
      methods: {
        ...
        plusOne: (event) => {
          ...
          let newVal = _gengi.vm.app[target];
        ...
        numPad: (event) => {
          if (!event.target.attributes.hasOwnProperty('key')) { return; }
          let key = event.target.attributes['key'].value;
          _gengi.numpad(key);
        },
      },
    });
  ...
  numpad: (key) => {
    let newVal = _gengi.vm.app[_gengi.vm.app.activeField].toString();
    let numpadValue = numpad(newVal, key);
  ...
  addToList: (currency) => {
    ...
    let currencies = _gengi.vm.currencies;
  ...
  showView: {
    ...
    calc: (options) => {
      ...
      _gengi.vm.app.amountISK = calculate(_gengi.vm.currencies.list[options.currency].rate, options.amount);
  ...
  initializeWatches: () => {
    _gengi.vm.$watch('app', () => {
      storage.set('app', _gengi.vm.app);
    }, {deep: true});
    _gengi.vm.$watch('currencies', () => {
      storage.set('currencies', _gengi.vm.currencies);
    }, {deep: true});
    _gengi.vm.$watch('currencyList', () => {
      storage.set('currencyList', _gengi.vm.currencyList);
    });
  ...
  initCurrencies: () => {
    let currencies = storage.get('currencies');
    ...
    promise.get(
      ...
      if (_gengi.vm.currencyList.length === 0) {
        let newList = [];
        for (let currency in res.currencies) {
  ...
  initData: (dataName) => {
    let data = storage.get(dataName) || _gengi.vm[dataName];
